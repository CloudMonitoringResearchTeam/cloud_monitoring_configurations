...
def conf_file(request):
    ...
    if category == 'prometheus_file':
        print('p')
        return read_or_write_prometheus_conf_file(request, ip)
    elif category == 'alert_rules':
        return read_or_write_alert_rule(request, ip)
    elif category == 'alert_file':

        return read_or_write_alert_conf_file(request, ip)

@csrf_exempt
def read_or_write_prometheus_conf_file(request, ip):
    if request.method == 'GET':

        node = get_object_or_404(Node, category='cluster',ip=ip)
...
def read_or_write_alert_rule(request, ip):
    if request.method == 'GET':
        node = get_object_or_404(Node, category='cluster',ip=ip)
        ssh = ssh_connect(node.ip, node.username, node.password)
...
def read_or_write_alert_conf_file(request, ip):
